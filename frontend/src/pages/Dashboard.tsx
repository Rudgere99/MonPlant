...
export default function Dashboard() {
  ...
  const WORK_HOURS_BASE = 24; // base operacional usada para meta/hora do Dashboard
  const [metaDia, setMetaDia] = useState<number>(8000);
  const [discountHours, setDiscountHours] = useState<number>(2); // mantido para compatibilidade com a API de metas

  const metaHorasTrabalhadas = useMemo(() => {
    // A média esperada do Dashboard não deve ser inflada por discount_hours.
    // Ex.: meta 5.000 t / 24 h = 208,33 t/h. O desconto fica salvo para planejamento, mas não infla o Dashboard.
    return WORK_HOURS_BASE;
  }, []);
