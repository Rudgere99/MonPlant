              ...
              {rows.map((r, idx) => {
                ...
                return (
                  <tr key={r.period} style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)" }}>
                    ...
                    <td style={{ padding: "10px 10px" }}>
                      <input
                        style={inputStyle}import React, {useEffect, useMemo, useState, useRef } from "react";
...
const [metaTon, setMetaTon] = useState<number>(4404);
const [discountHours, setDiscountHours] = useState<number>(2);
const [prodRows, setProdRows] = useState<PlantHourRow[]>([]);
const [periodSel, setPeriodSel] = useState<string>(() => {
  const h = new Date().getHours();
  const h2 = (h + 1) % 24;
  return `${pad2(h)}-${pad2(h2)}`;
});
const [tonPorConchada, setTonPorConchada] = useState<number>(2.78);

async function loadProductionResume() {
  if (!API_BASE) return;
  setProdLoading(true);
  try {
    const p = await apiGet<PlantDayPayload>(`/api/plant-production/${encodeURIComponent(day)}`).catch(() => ({ day, rows: [] } as any));
